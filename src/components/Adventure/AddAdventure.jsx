  ...
  const submitAlbum = async (data) => {
    try {
      ...
      if (adventureId) {
        ...
        toast.success("Update Adventure successful!", {
          position: "top-right",
        });
      } else {
        await addAdventures(formData).unwrap();
        toast.success("Adventure was successfuly added", {
          position: "top-right",
        });
      }
      reset();
  ...
  return (
    <div className="border border-black bg-white  rounded-lg p-3 max-w-4xl mx-auto my-10 ">
      <h3 className="text-xl font-bold text-center">
        {adventureId ? "Edit Adventure" : "Add new Adventure"}
      </h3>
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {fields.map((field, index) => (
            <div key={field.id}>
              ...
              <input
                className="border p-3 rounded-lg "
                {...register(`adventureType.${index}.price`, {
                  required: true,
                })}
